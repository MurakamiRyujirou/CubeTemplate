...
namespace MurakamiRyujirou.Cube
{
    ...
    {
        /// 回転操作を行う.
        public void Rotate(Cubie[,,] cubies, Operations oper)
        {
        ...
        /// 標準回転操作を行う.
        private void RotateInner(Cubie[,,] cubies, SingleOperations oper)
        {
        ...
        private void SwapCubies(Cubie[,,] cubies, Vector3Int[] p, bool clockwise)
        {
            if (clockwise)
            {
                Cubie temp = cubies[p[0].x, p[0].y, p[0].z];
            ...
            {
                Cubie temp = cubies[p[3].x, p[3].y, p[3].z];
        ...
        /// 指定した座標に存在するキュービーを回転させる.
        /// 回転は回転軸(axis)と、時計回りか否か(clockwise)で指定する.
        private void RotationCubies(Cubie[,,] cubies, Vector3Int[] pList, Axes axis, bool clockwise)
        {
            foreach (Vector3Int p in pList)
            {
                Cubie cubie = cubies[p.x, p.y, p.z];
